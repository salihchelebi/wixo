...
Türkçe derleme | [English](https://github.com/FlowiseAI/Flowise) | [繁體中文](./i18n/README-TW.md) | [简体中文](./i18n/README-ZH.md) | [日本語](./i18n/README-JA.md) | [한국어](./i18n/README-KR.md)
...
-   [🙌 Katkı](#-katkı)
-   [📄 Lisans](#-lisans)
-   [🧱 Monorepo Modülleri](#-monorepo-modülleri)
-   [🧪 Testler](#-testler)
-   [🏗️ Agentflow Mimarisi](#️-agentflow-mimarisi)
-   [📦 @flowiseaiagentflow Paketi](#-flowiseaiagentflow-paketi)
-   [🚀 Yayınlama](#-yayınlama)

## ⚡ Hızlı Başlangıç

[NodeJS](https://nodejs.org/en/download) `>= 18.15.0` sürümünü indirin ve kurun.

1. Flowise'i kurun
    ```bash
    npm install -g flowise
    ```
2. Flowise'i başlatın

    ```bash
    npx flowise start
    ```

3. [http://localhost:3000](http://localhost:3000) adresini açın.

## 🐳 Docker

### Docker Compose

1. Flowise projesini clone edin
2. Proje kökündeki `docker` klasörüne gidin
3. `.env.example` dosyasını kopyalayın, aynı konuma yapıştırın ve adını `.env` yapın
4. `docker compose up -d`
5. [http://localhost:3000](http://localhost:3000) adresini açın
6. Container'ları durdurmak için `docker compose stop` kullanın

### Docker Image

1. Image'ı yerelde build edin:

    ```bash
    docker build --no-cache -t flowise .
    ```

2. Image'ı çalıştırın:
...
2. Repository klasörüne girin:

    ```bash
    cd Flowise
    ```

3. Tüm modüllerin bağımlılıklarını kurun:

    ```bash
    pnpm install
    ```

4. Tüm kodu build edin:
...
FlowiseAI ekibinden bir üye, Pull Request açtığınızda otomatik olarak bilgilendirilir/atanır. İsterseniz [Discord](https://discord.gg/jbaHfsRVBW) üzerinden de ulaşabilirsiniz.

### Code of Conduct

Bu proje ve projeye katılan herkes, [CODE_OF_CONDUCT.md](CODE_OF_CONDUCT.md) dosyasında tanımlanan Code of Conduct'a tabidir. Katılımınızla birlikte bu kurallara uymanız beklenir. Uygun olmayan davranışları `hello@flowiseai.com` adresine bildirebilirsiniz.
...
- [ARCHITECTURE.md](./ARCHITECTURE.md) - İç mimari ve tasarım kalıpları
- [TESTS.md](./TESTS.md) - Test planı, coverage tier'ları ve konfigürasyon
- [Examples](./examples/README.md) - Kullanım örnekleri ve demolar

## 📄 Lisans

Bu repository içindeki kaynak kod [Apache License Version 2.0](LICENSE.md) altında sunulmaktadır.
