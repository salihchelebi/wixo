...
async function getStoredAssistantConfig() {
    await ensureSchema()
    const result = await query('SELECT config FROM wixoo_lite_assistant_config WHERE id = 1 LIMIT 1')
...
async function upsertAssistantConfig(config) {
    ...
    const result = await query(
        `INSERT INTO wixoo_lite_assistant_config (id, config, updated_at)
